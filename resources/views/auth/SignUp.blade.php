...
php
  ...
  if(isset($_POST['btnSubmit'])){
    ...
    $SELECT = "SELECT username FROM `tbluser` where username='$username'";
    $selectquery = $createConnection->query($SELECT) or die($createConnection->error);
    if(mysqli_num_rows($selectquery) != 1){
      $SQL="INSERT INTO `tbluser`(`username`, `password`) VALUES ('$username', '$password')";
      $Signup=$createConnection->query($SQL) or die($createConnection->error);
...
@section('title', 'Sign Up | JobSpeed')
    ...
    <div class="container-fluid">
      <div class="row" >
        ...
        <div class="CHECK signup col">
          ...
          <p>Already Have Account? <a class="hrefsignup" href="SignIn.blade.php">Sign In</a></p>
          <form action="{{ route('register-user') }}" method="post">
              ...
              <div class="col">
                <div class="form-floating">  
                  <input type="text" name="FirstName" placeholder="Enter First Name"
                    class=" form-control mb-1" value="{{ old('FirstName') }}" style="width:276px;">
                  <label for="FirstName">First Name</label>
                  <span class="text-danger ms-2">@error ('FirstName'){{ $message }} @enderror</span>
                </div>
              </div>
              <div class="col">
                <div class="form-floating">  
                  <input type="text" name="LastName" placeholder="Enter Last Name"
                    class=" form-control mb-1" value="{{ old('LastName') }}" style="width:280px; margin-left:-60px;">
                  <label for="LastName" style="margin-left:-60px;">Last Name</label>
                  <span class="text-danger ms-2">@error ('LastName'){{ $message }} @enderror</span>
                </div>
              </div>
            </div>
            ...
            <div class="form-floating">  
              <input type="email" name="email" placeholder="Enter Email"
                class="inputsignup form-control mb-1" value="{{ old('email') }}">
              <label for="email">Email</label>
              <span class="text-danger ms-2">@error ('email'){{ $message }} @enderror</span>
            </div>
            <div class="form-floating">
              <input type="password" name="password" placeholder="Enter Password"
                class="inputsignup form-control mb-1" value="{{ old('password') }}">
